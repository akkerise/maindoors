...
//        define('MERCHANT_PASS', 'matkhauketnoi'); // Mật khẩu kết nối
        $receiver = RECEIVER;
        $order_code = 'NL_'.time();
        $return_url = 'http://localhost:8000/nlreceiver';
        $cancel_url = 'http://localhost:8000';

        // Thông tin của cả giỏ hàng
...
//            $faker = Faker\Factory::create();
            ...
            $order->payment_type = 'Chuyển Khoản';
            $order->price = $price;
            $order->information = $order_description;
            $order->transaction_status = 1;
            $order->voucher_percent = 0;
            $order->voucher_money = 0;
            $order->customer_id = rand(1,10000);
            $order->transaction_info = $buyer_info;
            $order->payment_id = rand(10000000,99999999);
            $order->error_text = '';
...
//        return dd($url);
    ...
    public function nlReceiver(Request $request){
...
//            $transition_status = $request->transition_status;
            // Connect with Order talbe by Eloquent Model
//            dd($payment_id);
//            $new_order = Order::find($payment_id);
//            Trạng thái giao dịch: 1 – Giao dịch mới tạo, chưa thanh toán;
...
//            dd($new_order);
//            $new_order->transaction_status = 1;
//            dd($new_order);
//            dd($payment_id);
//            dd($transition_status);
            //Khai báo đối tượng của lớp NL_Checkout
            $nl= new NL_Checkout();
            $nl->merchant_site_code = MERCHANT_ID;
            $nl->secure_pass = MERCHANT_PASS;
